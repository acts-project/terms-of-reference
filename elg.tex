...
\usepackage[status=finalaft, version=1.0, name={ACTS Experiment Liaison Group}]{actsdoc}
...
The Experiment Liaison Group (ELG) is the standing channel connecting the developers of ACTS with the experiments that use and depend on the toolkit. It exists to:

\begin{itemize}
    \item Carry information in both directions: experiment needs, planned ACTS developments, and known issues.
...
The ELG holds no decision-making authority, the authority over the codebase remains with the maintainers; decisions internal to any experiment remain with that experiment. The group operates on the assumption of collaborative intent among all participants, and its purpose is to provide a forum, not arbitration.

Changes to supported platforms, compilers, or major dependencies are communicated to the ELG as an explicit point, though they do not require ELG approval. API consistency is maintained on a best-effort basis only and is not itself a matter for ELG sign-off. The release schedule is set by the project and is intentionally flexible. Dedicated release scheduling and maintenance considerations, e.g.~for experiments during data taking which will require long-term release support, should be brought up in the joint ELG-ACTS (or direct communication), and a support schedule involving both, the ACTS and experiment developers can be defined.
